...
class ObjectDetectionModel:
    ...
    def __init__(
        self,
        model_path: str = "mannequin-tent-cnn-mrr3.onnx",
    ...
    async def process_image(self, image_path: str) -> list[ObjectDetection]:
        ...
        def _onnx_callback(
            onnx_results: list[npt.NDArray[np.float32 | np.int64]],
            fut: asyncio.Future[ModelOutput],
            error: str,
        ) -> None:
            if error:
                loop.call_soon_threadsafe(fut.set_exception, ValueError(error))
                return
            loop.call_soon_threadsafe(fut.set_result, ModelOutput(onnx_results))
        ...
        model_output: ModelOutput = await future

        results = self._filter_output(model_output, (width, height))
